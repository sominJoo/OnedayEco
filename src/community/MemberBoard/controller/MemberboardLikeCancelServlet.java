...
/**
 * Servlet implementation class MemberboardLikeCancelServlet
 */
@WebServlet("/community/memberboardLikeCancel")
public class MemberboardLikeCancelServlet extends HttpServlet {
	...
	try {
		...
		memberboardService.updateMemberboardLikeCancel(no);
		// 3. 리다이렉트 & 사용자 피드백 (dml - 리다이렉트 필수)
		HttpSession session = request.getSession();
		session.setAttribute("clickLike", 0);
